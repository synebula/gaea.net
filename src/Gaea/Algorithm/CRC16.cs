namespace Gaea.Algorithm
{
    /// <summary>
    /// CRC16计算类。
    /// </summary>
    public class CRC16: IDigest<ushort>
    {
        #region CRCTable
        /// <summary>
        /// crc高位表
        /// </summary>
        private readonly byte[] auchCRCHigh = new byte[]
        {
            
        };

        /// <summary>
        /// crc低位表  
        /// </summary>
        private readonly byte[] auchCRCLow = new byte[]
        {
            
        };
        #endregion

        /// <summary>
        /// 计算一个byte数组的CRC16值。
        /// </summary>
        /// <param name="data">需要计算CRC16的byte数组。</param>
        /// <returns>返回CRC16值</returns>
        public ushort Calculate(byte[] bytes)
        {
            byte high = 0xff;  // 高位初始化  
            byte low = 0xff;  // 低位初始化  
            int index;
            for (int i = 0; i < bytes.Length; i++)
            {
                index = high ^ bytes[i]; //查找crc表值  
                high = (byte)(low ^ auchCRCHigh[index]);
                low = auchCRCLow[index];
            }
            return (ushort)(high << 8 | low);
        }

        /// <summary>
        /// 计算一个byte数组的CRC16值。
        /// </summary>
        /// <param name="bytes">需要计算CRC16的byte数组。</param>
        /// <param name="offset">开始位置。</param>
        /// <param name="length">长度。</param>
        /// <returns>返回CRC16值。</returns>
        public ushort Calculate(byte[] bytes, int offset, int length)
        {
            byte uchCRCHi = 0xFF; /* high byte of CRC initialized */
            byte uchCRCLo = 0xFF; /* low byte of CRC initialized */
            int index; /* will index into CRC lookup table */
            int len = length;
            int i = offset;
            while (len-- != 0) /* pass through message buffer */
            {
                index = uchCRCHi ^ bytes[i++]; /* calculate the CRC */
                uchCRCHi = (byte)(uchCRCLo ^ auchCRCHigh[index]);
                uchCRCLo = auchCRCLow[index];
            }

            return (ushort)(uchCRCHi << 8 | uchCRCLo);
        }
    }
}
